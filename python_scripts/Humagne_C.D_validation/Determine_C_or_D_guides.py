...
fastq_file_path = "/dunnstore/hassan/paul/HumagneC.D_validation/Fastq_files/combined_Humagne_C_and_D_reads.fastq"
...
def Determine_C_or_D_gRNAs(fastq_file_path):
    ...
    # List for the lines to be written into C
    c_fastq = []
    # List for the lines to be written into C
    d_fastq = []
    ...
    # opens a new file with the appropriate path (which is not Github as this will take up too much space)
    # as C
    with open("/dunnstore/hassan/paul/HumagneC.D_validation/Fastq_files/reads_Humagne_C.fastq","w") as C:
        # for each line in the newly created list it will add it to the newly created fastq file for Humagne C reads
        for line in c_fastq:
            C.write(line) # Write each line from c into C
    # same thing for Humagne D
